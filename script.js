...
const contentMap = { Home: "Hey Buddy!", About: "We are CuriousMinds, dedicated to learning.", Maths: "Master Mental Math with ease.", Science: "Explore the wonders of the universe.", Telugu: "Learn the Italian of the East.", Resources: "Worksheets and guides at your fingertips.", Contact: "Reach out to us anytime!" };

document.addEventListener('click', (e) => {
    if (e.target.id === 'menu-icon') document.querySelector('nav').classList.toggle('active');
    if (e.target.classList.contains('nav-link')) {
        document.querySelector('.hero h1').innerText = e.target.dataset.page;
        const p = document.querySelector('.hero p') || document.createElement('p');
        p.innerText = contentMap[e.target.dataset.page]; p.style.color = "#fff";
        if (!document.querySelector('.hero p')) document.querySelector('.hero').appendChild(p);
    }
...
const handleForm = async (id, action) => {
    ...
    form.onsubmit = async (e) => {
        ...
        if (action === 'login' && data.result === 'success') window.location.href = API_URL;
